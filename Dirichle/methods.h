...
float skn(float **A, float **B, int n, int m, int na, int ma)
{
    float sum = 0;
    for (int i = 1; i < na; i++)
        for (int j = 1; j < m; j++)
...
int Conjugate_gradient2(float **X, float **B, float e, int N, int n, int m,int na, int ma, float h, float k)
{
    int s = 0;
    float es = 100;
    float h2 = 1 / (h * h);
    ...
    while ((s < N) && (es > e))
    {
        ...
        float sk2 = skn(AZs, Zs, n ,m, na, ma);
        //Coeff!!!!!!
        float a = sk1 / sk2;
...
int Zeidel2(float **X, float **B, float e, int N, int n, int m,int na, int ma, float h, float k)
{
    int s = 0;
    float es = 100;
    float h2 = 1 / (h * h);
